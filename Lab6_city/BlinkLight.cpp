...
void BlinkLight::begin() {
  pinMode(_pin, OUTPUT);
} // begin assigns the pin to output uploaded functionality
...
void BlinkLight::update() {
  if (_enabled) { // Checks if the microcontroller enabled or disabled the
                  // lights
    if (_interval == 0) {
      digitalWrite(_pin, HIGH);
      return;
    } // If the interval is set to 0, then the LED light is enabled until off is
      // called
    ...
    if (now - _prevTime >= _interval) {
      ...
      _prevTime = now;
    } // compares RTOS milliseconds that have passed, and switches the state if
      // the difference in the time of the previous state and the time of the
      // current state
  }
}
